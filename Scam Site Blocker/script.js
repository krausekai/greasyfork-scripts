...
function main() {
	...
	// flag bad URL terms
	var badUrlTerms = ["call-now1", "call-now2", "mac-support", "p_num=", "call-for-microsoft-support", "/chx/", "/chmx/", "/AFUS", "/macx/", "/firedgex/", "ffiedgex", "firiedgemx", "blue.php", "iphonereport/t7", "network_error"];
	for (let i = 0; i < badUrlTerms.length; i++) {
		if (location.pathname.toLowerCase().includes(badUrlTerms[i].toLowerCase())) {
			//console.log("Web host flagged");
			redFlags++;
		}
	}
